...
class PremiumController extends Controller
{
    ...
    public function buyVip()
    {
        ...
        $premiumAccount =  cabalChangeAuth::where('UserNum', Auth::user()->UserNum)->first();
        ...
        else
        {
            ...
            if($premiumAccount->ServiceKind == 0)
            {
                ...
                DB::update( // POCAO INESGOTAVEL 1K HP
                    DB::raw("USE [CabalCash]exec  up_AddMyCashItemByItem '".Auth::user()->UserNum."', '1', '1', '7278', '0', '17' ")
                );
                DB::update( // POCAO INESGOTAVEL 1K MP
                    DB::raw("USE [CabalCash]exec  up_AddMyCashItemByItem '".Auth::user()->UserNum."', '1', '1', '7279', '0', '17' ")
                );
                DB::update( // DESTREINAMENTO DE PET
                    DB::raw("USE [CabalCash]exec  up_AddMyCashItemByItem '".Auth::user()->UserNum."', '1', '1', '7433', '10', '31' ")
                );
                ...
                DB::update( // NUCLEO GUIA
                    DB::raw("USE [CabalCash]exec  up_AddMyCashItemByItem '".Auth::user()->UserNum."', '1', '1', '5040', '0', '17' ")
                );
                ...
                return response()->json([
                    'success' => false,
                    'message' => "Você deve esperar seu vip vencer para poder renovado!",
                ]);
            }
        }
